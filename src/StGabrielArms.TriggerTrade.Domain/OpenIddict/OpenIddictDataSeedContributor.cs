...
namespace StGabrielArms.TriggerTrade.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("TriggerTrade") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "TriggerTrade", DisplayName = "TriggerTrade API", Resources = { "TriggerTrade" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["TriggerTrade_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["TriggerTrade_Web:RootUrl"]!.EnsureEndsWith('/');

            /* TriggerTrade_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["TriggerTrade_Web:ClientSecret"] ?? "1q2w3e*",
